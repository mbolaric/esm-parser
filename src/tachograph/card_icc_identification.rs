use crate::{
    Readable, gen1::SerialNumber, helpers::vec_u8_to_string, tacho::EmbedderIcAssemblerId,
};

#[derive(Debug)]
pub struct CardIccIdentification {
    pub clock_stop: u8,
    pub card_serial_number: SerialNumber,
...
impl Readable<CardIccIdentification> for CardIccIdentification {
    fn read<R: binary_data::ReadBytes + binary_data::BinSeek>(
        reader: &mut R,
    ) -> crate::Result<CardIccIdentification> {
        let clock_stop = reader.read_u8()?;
        let card_serial_number = SerialNumber::read(reader)?;

...
pub struct CardDriverActivityInfo {
    ...
    pub card_status: u16,
    pub driving_status: u16,
    pub driving_crew: DrivingCrew,
...
impl CardDriverActivityInfo {
    pub fn new(activity_card: ActivityCard, activity_info: u16) -> Self {
        let card_status = (activity_info >> 13) & 1;
        let driving_status = (activity_info >> 14) & 1;
        let driving_crew = CardDriverActivityInfo::get_driving_crew(card_status);
    ...
    fn get_activity_source(card_status: u16, driving_status: u16) -> ActivitySource {
        if card_status == 1 {
            if driving_status != 1 {
    ...
    fn get_activity_type(activity_card: &ActivityCard, activity_source: &ActivitySource, activity_info: u16) -> ActivityType {
        ...
        return ((activity_info >> 11) & 3).into();
    }
    ...
    fn get_card_slot(card_slot_valid: bool, activity_info: u16) -> CardSlotNumber {
        ...
        if ((activity_info >> 15) & 1) != 1 {
            return CardSlotNumber::Driver;
        }
        CardSlotNumber::CoDriver
    }

    fn get_slot_valid(activity_card: &ActivityCard, card_status: u16) -> bool {
    ...
    fn get_driving_crew(card_status: u16) -> DrivingCrew {
        if card_status == 1 {
            return DrivingCrew::Unknown;
        }
        if card_status == 0 {
            return DrivingCrew::Crew;
        }
        DrivingCrew::Single
    }
}
...
impl ReadableWithParams<CardDriverActivity> for CardDriverActivity {
    ...
    fn read<R: ReadBytes + BinSeek>(reader: &mut R, params: &Self::P) -> Result<CardDriverActivity> {
        ...
        loop {
            ...
            if activity_reader.pos()? == activity_pointer_newest_record as usize {
                break;
            }
        }
